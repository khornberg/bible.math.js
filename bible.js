...
class Bible {
  ...
  replaceRoman(str) {
    ...
    let matches = str.match(/((CM|CD)|(D)?(C){0,3})((XC|XL)|(L)?(X){0,3})((IX|IV)|(V)?(I){0,3})(?=(\s|[.,;:-])|$)/gi);
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
      let arabicNumeral = this.romanToArabic(matches[i]);
      let romanNumeral = new RegExp('\\b' + matches[i] + '(?=(\\s|[.,;:-])|$)', 'gi');
  ...
  romanToArabic(romanNumeral) {
    ...
    let rom = '^((CM|CD)|(D)?(C){0,3})((XC|XL)|(L)?(X){0,3})((IX|IV)|(V)?(I){0,3})$';
    let romanRegExp = new RegExp(rom, 'mi');
    if (romanRegExp.test(romanNumeral)) {
      let n = 0;
      for (let i = 0; i < romanNumerals.length; ++i) {
        for (
          let x = romanNumerals[i], l = x[1].length;
  ...
  add() {
    ...
    while (verses !== 0) {
      let chapterVerses = this.books[reference.bookIndex].verses[reference.chapter1];
      ...
      if (verses !== 0) {
        let nextChapter = reference.chapter1 + 1;
  ...
  subtract() {
    ...
    while (verses !== 0) {
      ...
      if (verses !== 0) {
        let previousChapter = reference.chapter1 - 1;
        //0 indexed array
  ...
  distance() {
    ...
    if (args.length == 1) {
      ...
      // chapter1 and chapter2 are whole chapters
        ...
        for (let ch = args[0].chapter2; ch >= args[0].chapter1; ch--) {
